...
def _get_model(config: AgentSettings):
    ...
    if model_id.startswith("ollama:"):
        ...
        return OpenAIChatModel(base_id, provider=provider)

    # Handle Bedrock specifically for the region
    if model_id.startswith("bedrock:"):
        base_id = model_id.replace("bedrock:", "")
        provider = BedrockProvider(region_name=config.aws.region)
        return BedrockConverseModel(base_id, provider=provider)
